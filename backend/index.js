...
const URL_MERRA2_OPENDAP = "https://opendap.earthdata.nasa.gov/collections/C1276812863-GES_DISC/granules/M2T1NXSLV.5.12.4%3AMERRA2_100.tavg1_2d_slv_Nx.19800101.nc4.dap.nc4?dap4.ce=/QV2M;/T2M;/T2MDEW;/U10M;/V10M;/time;/lat;/lon";
...
const CONFIG_DATOS_NASA = {
    ...
    ventoso: { variableName: 'U10M_V10M', units: 'm/s', apiUrl: URL_MERRA2_OPENDAP },
    incomodo: { variableName: 'T2M_T2MDEW_QV2M', units: 'Índice (Combinado)', apiUrl: URL_MERRA2_OPENDAP },
    humedo: {
...
const processNasaHistoricalData = async (lat, lon, day, month, variable) => {
    ...
    if (!config) throw new Error(`[Error Backend] Variable '${variable}' no soportada. Asegúrate de que el frontend usa las claves en español.`);
    ...
    switch (variable) {
        ...
        case 'ventoso':
            historicalMean = 7.0 + (Math.abs(latNum) / 90 * 3) + (monthNum >= 10 || monthNum <= 3 ? 2.0 : 0.0);
            threshold = 10.0;
        ...
        case 'polvo':
            historicalMean = 0.05 + (Math.abs(latNum) < 30 && monthNum >= 3 && monthNum <= 5 ? 0.2 : 0.0);
            threshold = 0.5;
