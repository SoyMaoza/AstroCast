...
import { EstadisticasClima } from "../models/Clima.js";
// --- CONFIGURACIÓN ---
 ...
 */
export async function obtenerEstadisticasHistoricas(diaDelAnio, anioInicio, anioFin) {
...
const promesas = [];
  for (let anio = anioInicio; anio <= anioFin; anio++) {
    ...
    promesas.push(obtenerDatosParaUnDia(fecha));
  }
  ...
  return new EstadisticasClima(UBICACION.nombre, diaDelAnio, temperaturas, vientos, precipitaciones, aerosoles);
}
/**
 ...
 */
async function obtenerDatosParaUnDia(fecha) {
  try {
    const [tempData, precipData, vientoHumData, aireData] = await Promise.all([
      getDatosTemperatura(fecha),
      getDatosPrecipitacion(fecha),
      getDatosVientoHumedad(fecha),
      getDatosCalidadAire(fecha)
    ]);
    return {
      fecha,
...
async function getDatosTemperatura(fecha) {
  ...
  const { latIndex, lonIndex } = encontrarIndiceGridMasCercano(lats, lons);
...
async function getDatosPrecipitacion(fecha) {
...
async function getDatosVientoHumedad(fecha) {
...
async function getDatosCalidadAire(fecha) {
...
async function buscarYDescargarGranulo(short_name, fecha) {
  ...
  if (!respBusqueda.ok) throw new Error('API Error en búsqueda (${short_name}): ${respBusqueda.status}');
  const datos = await respBusqueda.json();
  if (datos.feed.entry.length === 0) throw new Error('No se encontró gránulo para ${short_name} en ${fecha}');
  ...
  if (!respArchivo.ok) throw new Error('Error descargando archivo desde ${enlace}');
  const buffer = await respArchivo.arrayBuffer();
  return { buffer, url: enlace };
}
function encontrarIndiceGridMasCercano(lats, lons) {
